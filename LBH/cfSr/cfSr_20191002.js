...
/*  History
 ...
 -----------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    2018-07-29          Faouzi Ben Mabrouk              Machine condition for Carousel Machine
    2018-10-01          Faouzi Ben Mabrouk              Extended to handle Machine condition for SIIF
...
function formatName(name,l){
    var n=name;
    if (name.length<l) {
        for (var i=name.length;i<=l;i++) n += " ";
    }
    return n;
}
...
function cfSrGetAvailablePositions(jsonData) {
    ...
    try {
        ...
        if (stationNumber.equals("06008010")) {
            machineName = "SIIF C06";
            numberOfPositions = 6; 
            mode = 1;
        }
        if (stationNumber.equals("06008020")) {
            machineName = "SIIF C10";
            numberOfPositions = 6; 
            mode = 1;
        }
        // -----------------------------------------------------------------------
        ...
        // mode = 1 : SIIF Process
        var Positions = ["Cooling","Sawing 1","Sawing 2","Decoring 1","Decoring 2","R3"];
        ...
        if (mode == 1) {
            for (var i=0;i<numberOfPositions;i++){
                ...
                for (var j=1;j<2;j++){
                    ...
                    var st = getSiifStationId(stationNumber,i+1).station;
                    stations.push(st);
                ...
                position = {position:formatName(Positions[i],12),stationNumbers:stations,colorRgb:colorRgb,state:"1"}; 
                if ((position.colorRgb==undefined) || (colorRgb=undefined)) {
                    colorRgb="224;224;224;255";
                    position = {position:formatName(Positions[i],12),stationNumbers:stations,colorRgb:colorRgb,state:"1"}; 
                }
...
// #############################################################################
function getSiifStationId(station, id) {
    var temp = "";

    var l = station.length;

    st = station.substring(0, l - 1);
